    ...
    const renderCarousel = (slides) => (
        ...
        >
            {slides.map((slide, idx) => {
                const cleanTitle = slide.title.replace(slide.highlight, '').trim();

                return (
                    <div key={`banner-slide-${idx}`} className="relative">
                        <img
                            src={slide.img}
                            alt={`banner ${slide.title}`}
                        ...
                        <div className="absolute inset-0 flex flex-col items-center justify-center px-6 text-center rounded-lg bg-black/40">
                            <p className="mb-3 text-lg font-semibold text-red-600">
                                Khuyến mãi đặc biệt{' '}
                                <span className={`px-2 py-[2px] text-white ${slide.offerBgColor} rounded-full text-sm`}>
                                    {slide.offer}
                                </span>
                            </p>
                            ...
                            <h2 className={`mb-6 text-2xl md:text-4xl font-bold ${slide.titleColor}`}>
                                {cleanTitle}
                                <span className={`block ${slide.highlightColor}`}>{slide.highlight}</span>
                            </h2>

                            <Link
                                to="/all-products"
                                className="px-4 py-2 text-sm font-semibold text-green-500 transition bg-white border-2 rounded-lg border-gradient-to-r from-green-400 to-blue-500 hover:text-white hover:bg-gradient-to-r hover:from-green-400 hover:to-blue-500"
                            >
                                Xem Ngay →
                            </Link>
                        </div>
