        ...
        <div className="container p-4 pt-20 mx-auto">
            ...
            <div className="flex-wrap justify-center hidden gap-4 px-2 py-3 lg:flex">
                {loading
                    ? placeholders.map((_, idx) => (
                          <div
                              key={`placeholder-${idx}`}
                              className="w-24 h-24 bg-gray-200 rounded-full md:w-20 md:h-20 lg:w-24 lg:h-24 animate-pulse"
                          />
                      ))
                    : categories.map((cat) => (
                          <Link
                              key={cat.category}
                              to={`/product-category?category=${encodeURIComponent(cat.category)}`}
                          ...
                          >
                              <div className="flex items-center justify-center w-24 h-24 overflow-hidden bg-gray-100 rounded-full shadow-md md:w-20 md:h-20 lg:w-24 lg:h-24">
                                  <img
                                      src={cat.productImage?.[0] || '/placeholder.png'}
            ...
            <div className="relative flex items-center lg:hidden">
                <button
                    type="button"
                    onClick={scrollLeft}
                    className="absolute z-10 p-2 -translate-y-1/2 rounded-full shadow-md top-1/2 left-2 bg-white/70 hover:bg-white"
                >
                ...
                <div
                    ref={carouselRef}
                    className="flex gap-6 px-8 py-4 overflow-x-auto whitespace-nowrap scroll-smooth cursor-grab no-scrollbar"
                >
                    {(loading ? placeholders : [...categories, ...categories]).map((cat, idx) =>
                        loading ? (
                            ...
                            <Link
                                key={`mobile-cat-${idx}-${cat.category}`}
                                to={`/product-category?category=${encodeURIComponent(cat.category)}`}
                ...
                <button
                    type="button"
                    onClick={scrollRight}
                    className="absolute z-10 p-2 -translate-y-1/2 rounded-full shadow-md top-1/2 right-2 bg-white/70 hover:bg-white"
                >
