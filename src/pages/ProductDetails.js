            ...
            <div className="flex flex-col md:flex-row gap-6 min-h-[300px]">
                            ...
                            <>
                                ...
                                {zoomImage && (
                                    <div className="hidden lg:block absolute -right-[420px] top-0 w-[400px] h-[400px] border border-gray-300 overflow-hidden bg-white">
                                        <div
                                            className="w-full h-full scale-150 bg-no-repeat bg-contain"
                                            style={{
                    ...
                    <div className="flex gap-2 overflow-x-auto md:flex-col md:overflow-x-hidden scrollbar-none">
                        {loading
                            ? productImageListLoading.map((_, i) => (
                                  <div
                                      key={i}
                                      className="w-20 h-20 rounded lg:w-20 lg:h-20 bg-slate-200 animate-pulse"
                                  />
                              ))
                            : data?.productImage?.map((imgURL, idx) => (
                                  <div
                                      key={idx}
                                      className={`w-20 h-20 p-1 rounded cursor-pointer border ${
                                          activeImage === imgURL ? 'border-indigo-500' : 'border-gray-300'
                                      }`}
                                  >
                                      <img
                                          src={imgURL}
                ...
                <div className="flex flex-col w-full gap-3 md:w-1/2">
                    {loading ? (
                        ...
                        <>
                            <p className="inline-block px-2 py-1 text-red-600 bg-red-200 rounded-full w-fit">
                                {data?.brandName}
                            </p>
